...
test('matrix browser: frozen columns, responsive, filters and green/yellow/red workflows',{skip:!enabled||!existsSync(chrome),timeout:60000},async()=>{
 ...
 try{
  ...
  assert.deepEqual(await evaluate("[...document.querySelectorAll('.matrix-table th')].slice(0,5).map(x=>x.getBoundingClientRect().width)"),[60,100,54,140,56]);
  assert.equal(await evaluate("document.querySelector('.matrix-table tbody td').textContent"),'CAKE');
  assert.equal(await evaluate("document.querySelector('.matrix-table tbody td:nth-child(3)').textContent"),'VISA');
  ...
  assert.ok(await evaluate("document.querySelector('.matrix-table td:nth-child(4)').title.endsWith('hai dòng')"));
