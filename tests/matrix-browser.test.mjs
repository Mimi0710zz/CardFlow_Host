...
test('matrix browser: frozen columns, responsive, filters and green/yellow/red workflows',{skip:!enabled||!existsSync(chrome),timeout:60000},async()=>{
 ...
 try{
  ...
  assert.deepEqual(await evaluate("(()=>{const header=[...document.querySelectorAll('.matrix-header-name')].find(node=>node.textContent==='N.Q.Minh');return {tag:header?.tagName,parent:header?.parentElement?.tagName,parentTitle:header?.parentElement?.title,color:getComputedStyle(header).color};})()"),{tag:'SPAN',parent:'TH',parentTitle:'Nguyễn Quang Minh',color:'rgb(31, 78, 120)'});
