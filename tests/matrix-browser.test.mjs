...
test('matrix browser: frozen columns, responsive, filters and green/yellow/red workflows',{skip:!enabled||!existsSync(chrome),timeout:60000},async()=>{
 ...
 repaint();document.querySelector('[data-coord-tab="matrix"]').click();window.ready=true;
 </script></body></html>`;
 ...
 try{
  ...
  assert.deepEqual(await evaluate("[...document.querySelectorAll('.matrix-table th')].slice(0,4).map(x=>x.textContent)"),['Ngân hàng','Card ID','Phôi','Chương trình hoàn tiền']);
  const geometry=await evaluate(`(()=>{const wrap=document.querySelector('.matrix-scroll'),cells=[...document.querySelectorAll('.matrix-table tbody td')].slice(0,4),before=cells.map(x=>x.getBoundingClientRect().x);wrap.scrollLeft=400;return {before,after:cells.map(x=>x.getBoundingClientRect().x),width:document.querySelector('.matrix-table th:nth-child(5)').getBoundingClientRect().width};})()`);
  assert.deepEqual(geometry.before,geometry.after);assert.ok(geometry.width>=70&&geometry.width<=90);
  ...
  await evaluate("document.querySelector('[data-matrix-action]').click()");
  ...
  await evaluate("document.querySelector('[data-matrix-action]').click()");assert.equal(await evaluate("document.querySelector('.feature-modal [name=customerId]').value"),'c1');
  ...
  await evaluate("document.querySelector('[data-matrix-action]').click()");assert.equal(await evaluate("document.querySelectorAll('[data-feature-table=transaction-order] tbody tr').length"),2);
  assert.match(await evaluate("document.querySelector('#view-transactions').textContent"),/23-08-2026/);
  await evaluate("document.querySelector('#view-transactions').hidden=true;document.querySelector('#view-coordination').hidden=false;document.querySelector('[data-matrix-actionable]').click()");assert.equal(await evaluate("document.querySelectorAll('.matrix-scroll .COMPLETED').length"),0);
  await evaluate("document.querySelector('[data-matrix-clear]').click();document.querySelector('.matrix-customer-picker').open=true;document.querySelector('[data-matrix-none]').click()");assert.match(await evaluate("document.querySelector('[data-matrix-content]').textContent"),/Không có/);
  await evaluate("document.querySelector('[data-matrix-all]').click();document.querySelector('.matrix-customer-picker').open=false");
  ...
  if(process.env.MATRIX_SCREENSHOT_DIR){await send('Emulation.setDeviceMetricsOverride',{width:1440,height:1000,deviceScaleFactor:1,mobile:false});const shot=await send('Page.captureScreenshot',{format:'png'});writeFileSync(join(process.env.MATRIX_SCREENSHOT_DIR,'matrix-desktop.png'),Buffer.from(shot.data,'base64'));await send('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:false});const mobile=await send('Page.captureScreenshot',{format:'png'});writeFileSync(join(process.env.MATRIX_SCREENSHOT_DIR,'matrix-mobile.png'),Buffer.from(mobile.data,'base64'));}
