...
function renderCatalog(){const rows=state.banks.slice().sort((a,b)=>a.name.localeCompare(b.name,"vi")).map(b=>`<tr data-id="${b.id}">${cell("Mã ngân hàng",esc(b.code))}${cell("Tên ngân hàng",esc(b.name))}</tr>`);$("#view-catalog").innerHTML=`<div class="panel"><h2>Mã ngân hàng</h2><div class="toolbar"><button class="primary" data-add="bank">Thêm</button><button data-edit-selected="bank">Tùy chỉnh</button><button class="danger" data-delete-selected="bank">Xóa</button></div>${entityTable(["Mã ngân hàng","Tên ngân hàng"],rows,"bank")}</div>`;}
...
function renderAbout(){
  const guides=hostGuideItems(),activeTab=sessionStorage.getItem("cardflow-host-about-tab")==="guide"?"guide":"intro";
...
function openForm(entity,id=null,preset={}){const modal=$("#modal"),form=$("form",modal),body=$(".modal-body",modal);form.dataset.entity=entity;form.dataset.id=id||"";modal.classList.toggle("customer-editor-modal",entity==="customer");let item;
  ...
  if(entity==="product"){item=state.cardProducts.find(x=>x.id===id)||preset;$("h2",modal).textContent=id?"Tùy chỉnh Thẻ ngân hàng":"Thêm Thẻ ngân hàng";body.innerHTML=field("cardId","Card ID",item.cardId,"text",true)+selectField("bankId","Mã ngân hàng",bankOptions(item.bankId),true)+field("cardName","Tên thẻ",item.cardName,"text",true)+selectField("cardRank","Hạng thẻ",rankOptions(item.cardRank||"Classic/Standard"),true)+selectField("ownershipType","Loại thẻ",OWNERSHIP_TYPES.map(value=>`<option value="${value}" ${value===(item.ownershipType||"credit")?"selected":""}>${ownershipTypeLabel(value)}</option>`).join(""),true)+selectField("cardBrand","Mạng thẻ",textValueOptions(cardBrandValues(),item.cardBrand||item.network||""),true)+selectField("cardForm","Hình thức thẻ",textValueOptions(CARD_FORMS,item.cardForm||"Vật lý"),true)+selectField("cashbackCycleMode","Hình thức hoàn",`<option value="monthly" ${item.cashbackCycleMode!=="statement"?"selected":""}>Theo tháng</option><option value="statement" ${item.cashbackCycleMode==="statement"?"selected":""}>Theo sao kê</option>`,true)+selectField("defaultStatementDay","Ngày sao kê mặc định",dayOptions(item.defaultStatementDay))+selectField("status","Trạng thái",statusOptions(item.status))+noteField(item.notes);}
  if(entity==="link"){item=state.customerCards.find(x=>x.id===id)||preset;$("h2",modal).textContent=id?"Tùy chỉnh thẻ khách hàng":"Thêm thẻ cho khách hàng";body.innerHTML=selectField("customerId","Khách hàng",customerOptions(item.customerId),true)+selectField("cardProductId","Thẻ ngân hàng",customerCardProductOptions(item.cardProductId),true)+field("creditLimit","Hạn mức",formatVndInput(item.creditLimit||0),"text",true,"inputmode=numeric")+selectField("cashbackCycleModeOverride","Hình thức hoàn ngoại lệ",`<option value="">Kế thừa từ Thẻ ngân hàng</option><option value="monthly" ${item.cashbackCycleModeOverride==="monthly"?"selected":""}>Theo tháng</option><option value="statement" ${item.cashbackCycleModeOverride==="statement"?"selected":""}>Theo sao kê</option>`)+selectField("statementDay","Ngày sao kê",dayOptions(item.statementDay))+selectField("paymentDueDay","Ngày đến hạn",dayOptions(item.paymentDueDay))+field("openingDate","Ngày mở thẻ",item.openingDate,"date")+field("expiryDate","Ngày hết hạn",item.expiryDate,"date")+field("last4Digits","4 số cuối",item.last4Digits,"text",false,"inputmode=numeric maxlength=4 pattern=\\d{0,4}")+selectField("status","Trạng thái",statusOptions(item.status))+noteField(item.notes);bindVndInput($('[name="creditLimit"]',body));}
...
function submitForm(event){event.preventDefault();const form=event.currentTarget,entity=form.dataset.entity,id=form.dataset.id,data=Object.fromEntries(new FormData(form));const now=new Date().toISOString();
  ...
  if(entity==="product"){if(state.cardProducts.some(x=>x.cardId.toLowerCase()===data.cardId.trim().toLowerCase()&&x.id!==id))return toast("Card ID đã tồn tại");if(data.cashbackCycleMode==="statement"&&!data.defaultStatementDay)return toast("Theo sao kê yêu cầu Ngày sao kê mặc định");const ownershipType=normalizeOwnershipType(data.ownershipType);upsert("cardProducts",id,{...data,defaultStatementDay:data.defaultStatementDay?Number(data.defaultStatementDay):"",cardId:data.cardId.trim(),cardName:data.cardName.trim(),cardRank:normalizeCardRank(data.cardRank),ownershipType,cardType:ownershipType,cardBrand:normalizeCardBrand(data.cardBrand),cardForm:data.cardForm.trim(),updatedAt:now});}
