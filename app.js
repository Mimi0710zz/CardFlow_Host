import {LocalRepository,uuid} from "./services/local-repository.js?v=20260830-cardtypesv6";
...
function renderCards(){
  ...
  const rows=items.map(p=>{const links=linksForProduct(p.id),analysis=attributed.get(p.id)||{total:0,inconsistencies:[]};return `<tr data-id="${p.id}">${cell("Card ID",esc(p.cardId))}${cell("Ngân hàng",esc(bank(p.bankId)?.name||"—"))}${cell("Tên thẻ",`<strong>${esc(p.cardName)}</strong>`)}${cell("Loại thẻ",esc(p.cardBrand||p.network||"—"))}${cell("Hình thức thẻ",esc(p.cardForm||"—"))}${cell("Số khách sở hữu",links.length)}${cell("Tổng hạn mức",effectiveLimitDisplay(analysis))}${cell("Hạn mức trung bình",analysis.inconsistencies.length?effectiveLimitDisplay(analysis):formatMoney(links.length?analysis.total/links.length:0))}${cell("Ghi chú",esc(p.notes||"—"))}</tr>`;});
...
function hostGuideItems(){return [
  ...
  {icon:"credit-card",title:"Quản lý danh sách Thẻ",summary:"Cấu hình Card master trước khi gán cho khách hàng.",html:`<p>Tab <strong>Thẻ</strong> là nguồn dữ liệu master. Các trường hiện có gồm Card ID, Ngân hàng, Tên thẻ, Loại thẻ, Hình thức thẻ, Trạng thái và Ghi chú.</p><p>Loại thẻ hỗ trợ: <strong>American Express, JCB, MasterCard, NAPAS, Union Pay, Visa</strong>. Card ID phải duy nhất; thông tin master được các màn hình Khách hàng tham chiếu lại.</p>`},
...
function submitForm(event){event.preventDefault();const form=event.currentTarget,entity=form.dataset.entity,id=form.dataset.id,data=Object.fromEntries(new FormData(form));const now=new Date().toISOString();
  ...
  if(entity==="product"){if(state.cardProducts.some(x=>x.cardId.toLowerCase()===data.cardId.trim().toLowerCase()&&x.id!==id))return toast("Card ID đã tồn tại");upsert("cardProducts",id,{...data,cardId:data.cardId.trim(),cardName:data.cardName.trim(),cardBrand:normalizeCardBrand(data.cardBrand),cardForm:data.cardForm.trim(),updatedAt:now});}
