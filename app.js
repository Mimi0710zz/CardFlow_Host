...
function renderDashboard(){
  ...
  <div class="grid-2 dashboard-rankings"><div class="panel"><h2>Top 10 thẻ có nhiều khách hàng nhất</h2>${simpleTable(["Card ID","Ngân hàng","Số khách sở hữu"],topProducts.map(item=>[`<strong>${esc(item.cardId)}</strong>`,esc(item.bankName||"—"),item.count]),"dashboard-ranking-table")}</div><div class="panel"><h2>Top 10 khách hàng nhiều thẻ nhất</h2>${simpleTable(["Khách hàng","Số thẻ","Tổng hạn mức"],topCustomers.map(item=>[`<a href="#" data-open-customer="${item.id}">${esc(item.name)}</a>`,item.count,effectiveLimitDisplay(item.analysis)]),"dashboard-ranking-table")}</div></div>`;
  $$('[data-open-customer]').forEach(x=>x.onclick=e=>{e.preventDefault();openCustomerDetail(x.dataset.openCustomer);});
}
function simpleTable(headers,rows,className=""){return rows.length?`<div class="table-wrap"><table class="${className}"><thead><tr>${headers.map(x=>`<th>${x}</th>`).join("")}</tr></thead><tbody>${rows.map(row=>`<tr>${row.map(x=>`<td>${x}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`:`<div class="empty">Chưa có dữ liệu.</div>`;}
