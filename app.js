...
import {renderCashbackFeatures,renderCashbackDashboard} from "./services/cashback-feature-ui.js?v=20260831-host-compact-filter-v7";
...
function renderCards(){
  ...
  $("#view-cards").innerHTML=`<div class="panel"><div class="section-title"><h2>Thẻ ngân hàng</h2></div><div class="toolbar toolbar-zoned card-toolbar"><div class="toolbar-left"><input class="search" data-filter="cards.q" placeholder="Tìm Card ID, ngân hàng, hạng thẻ..." value="${esc(f.q||"")}"></div><div class="toolbar-center">${filterSelect("cards.bank","Ngân hàng",state.banks,f.bank,x=>x.name)}${plainFilter("cards.rank","Hạng thẻ",CARD_RANKS.map(x=>`${x}|${x}`),f.rank,true)}${plainFilter("cards.ownership","Loại thẻ",OWNERSHIP_TYPES.map(x=>`${x}|${ownershipTypeLabel(x)}`),f.ownership)}${plainFilter("cards.brand","Phôi",cardBrandValues().map(x=>`${x}|${x}`),f.brand)}${plainFilter("cards.form","Hình thức thẻ",CARD_FORMS.map(x=>`${x}|${x}`),f.form)}</div><div class="toolbar-right"><button class="primary" data-add="product">Thêm</button><button data-edit-selected="product">Tùy chỉnh</button><button class="danger" data-delete-selected="product">Xóa</button></div></div>${entityTable(["Card ID","Loại thẻ","Ngân hàng","Tên thẻ","Hạng thẻ","Phôi","Hình thức thẻ","Hình thức hoàn","Số khách sở hữu","Ghi chú"],rows,"product")}</div>`;
}
...
function openForm(entity,id=null,preset={}){const modal=$("#modal"),form=$("form",modal),body=$(".modal-body",modal);form.dataset.entity=entity;form.dataset.id=id||"";modal.classList.toggle("customer-editor-modal",entity==="customer");let item;
  ...
  if(entity==="product"){item=state.cardProducts.find(x=>x.id===id)||preset;$("h2",modal).textContent=id?"Tùy chỉnh Thẻ ngân hàng":"Thêm Thẻ ngân hàng";body.innerHTML=field("cardId","Card ID",item.cardId,"text",true)+selectField("bankId","Mã ngân hàng",bankOptions(item.bankId),true)+field("cardName","Tên thẻ",item.cardName,"text",true)+selectField("cardRank","Hạng thẻ",rankOptions(item.cardRank||"Classic/Standard"),true)+selectField("ownershipType","Loại thẻ",OWNERSHIP_TYPES.map(value=>`<option value="${value}" ${value===(item.ownershipType||"credit")?"selected":""}>${ownershipTypeLabel(value)}</option>`).join(""),true)+selectField("cardBrand","Phôi",textValueOptions(cardBrandValues(),item.cardBrand||item.network||""),true)+selectField("cardForm","Hình thức thẻ",textValueOptions(CARD_FORMS,item.cardForm||"Vật lý"),true)+selectField("cashbackCycleMode","Hình thức hoàn",`<option value="monthly" ${item.cashbackCycleMode!=="statement"?"selected":""}>Hoàn theo tháng</option><option value="statement" ${item.cashbackCycleMode==="statement"?"selected":""}>Hoàn theo sao kê</option>`,true)+selectField("defaultStatementDay","Ngày sao kê mặc định",dayOptions(item.defaultStatementDay))+selectField("status","Trạng thái",statusOptions(item.status))+noteField(item.notes);}
...
function openCustomerDetail(id){
  ...
  const rows=displayRows.map(({link:l,card:p,bankName})=>`<tr data-product-link="${p.id}"><td data-label="Card ID"><strong>${esc(p.cardId||"—")}</strong></td>${cell("Ngân hàng",esc(bankName||"—"))}${cell("Hạng thẻ",esc(p.cardRank))}${cell("Phôi",esc(p.cardBrand||p.network||l.cardBrand||"—"))}${cell("Hình thức hoàn",esc(cashbackCycleModeLabel(l.cashbackCycleModeOverride||p.cashbackCycleMode)))}${cell("Loại thẻ",ownershipTypeLabel(p.ownershipType))}${cell("Hạn mức",formatMoney(l.creditLimit))}${cell("Sao kê",formatDay(l.statementDay))}${cell("Đến hạn",formatDay(l.paymentDueDay))}${cell("Chung hạn mức",l.sharedLimitCardIds?.length?`${l.sharedLimitCardIds.length+1} thẻ cùng nhóm`:"Không")}${cell("Trạng thái",badge(l.status))}</tr>`);
