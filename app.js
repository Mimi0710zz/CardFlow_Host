import {LocalRepository,uuid,DEFAULT_ORDER_TYPE_COLORS,normalizeColor,orderTypeDefaultColor} from "./services/local-repository.js?v=20260902-order-type-color-v1";
...
import {compareText,sortByLabel,compareCards,compareCardId,compareCustomerCardLinks,buildSortedCustomerCardRows,compareCustomers,compareCustomerCode} from "./services/sorting.js?v=20260917-customer-code-sortv1";
...
import {renderCashbackFeatures,renderCashbackDashboard} from "./services/cashback-feature-ui.js?v=20260907-matrix-summary-resize-v2";
...
function bindColorPickers(root=document){
  ...
  root.querySelectorAll("[data-word-color-picker]").forEach(field=>{
    ...
    if(!value||!trigger||!popover||!moreDialog)return;
    ...
    const apply=color=>{
      ...
      const text=trigger.querySelector("span:not(.word-color-caret)");if(text)text.textContent=safe;
  ...
  colorPickerOutsideCleanup=()=>{cleanups.splice(0).forEach(cleanup=>cleanup());};
}
...
function renderCustomers(){
  ...
  items.sort(compareCustomerCode);
...
function renderOrderTypes(){const rows=[...(state.orderTypes||[])].sort((a,b)=>String(a.code||"").localeCompare(String(b.code||""),"vi",{numeric:true,sensitivity:"base"})).map(item=>`<tr data-id="${esc(item.id)}">${cell("Mã loại đơn",`<strong>${esc(item.code)}</strong>`)}${cell("Màu",orderTypeColorCell(item))}${cell("Mô tả",esc(item.description||"—"))}${cell("Ghi chú",esc(item.note||"—"))}</tr>`);$("#view-order-types").innerHTML=`<div class="panel"><h2>Loại đơn</h2><div class="toolbar"><button class="primary" data-add="orderType">Thêm</button><button data-edit-selected="orderType">Tùy chỉnh</button><button class="danger" data-delete-selected="orderType">Xóa</button></div>${entityTable(["Mã loại đơn","Màu","Mô tả","Ghi chú"],rows,"orderType","Chưa có loại đơn.")}</div>`;}
function renderSourceNames(){const rows=[...(state.sourceNames||[])].sort((a,b)=>String(a.name||"").localeCompare(String(b.name||""),"vi",{numeric:true,sensitivity:"base"})).map(item=>`<tr data-id="${esc(item.id)}">${cell("Tên nguồn",`<strong>${esc(item.name)}</strong>`)}${cell("Mô tả",esc(item.description||"—"))}${cell("Ghi chú",esc(item.note||"—"))}</tr>`);$("#view-source-names").innerHTML=`<div class="panel"><h2>Tên nguồn</h2><div class="toolbar"><button class="primary" data-add="sourceName">Thêm</button><button data-edit-selected="sourceName">Tùy chỉnh</button><button class="danger" data-delete-selected="sourceName">Xóa</button></div>${entityTable(["Tên nguồn","Mô tả","Ghi chú"],rows,"sourceName","Chưa có tên nguồn.")}</div>`;}
...
function openForm(entity,id=null,preset={}){const modal=$("#modal"),form=$("form",modal),body=$(".modal-body",modal);form.dataset.entity=entity;form.dataset.id=id||"";modal.classList.toggle("customer-editor-modal",entity==="customer");form.classList.toggle("order-type-modal",entity==="orderType");let item;
  ...
  if(entity==="sourceName"){item=(state.sourceNames||[]).find(x=>x.id===id)||preset;$("h2",modal).textContent=id?"Tùy chỉnh Tên nguồn":"Thêm Tên nguồn";body.innerHTML=field("name","Tên nguồn",item.name,"text",true)+field("description","Mô tả",item.description)+`<div class="field full"><label>Ghi chú</label><textarea name="note">${esc(item.note||"")}</textarea></div>`;}
  if(entity==="orderType"){item=(state.orderTypes||[]).find(x=>x.id===id)||preset;$("h2",modal).textContent=id?"Tùy chỉnh Loại đơn":"Thêm Loại đơn";body.innerHTML=field("code","Mã loại đơn",item.code,"text",true)+colorPickerField("color","Màu",item.color||orderTypeDefaultColor(item.code))+field("description","Mô tả",item.description)+`<div class="field full"><label>Ghi chú</label><textarea name="note">${esc(item.note||"")}</textarea></div>`;}
  bindColorPickers(body);modal.classList.add("show");setTimeout(()=>$("input,select",body)?.focus(),0);
}
...
function submitForm(event){event.preventDefault();const form=event.currentTarget,entity=form.dataset.entity,id=form.dataset.id,data=Object.fromEntries(new FormData(form));const now=new Date().toISOString();
  ...
  if(entity==="sourceName"){const name=String(data.name||"").trim();if(!name)return toast("Tên nguồn là bắt buộc");if((state.sourceNames||[]).some(x=>String(x.name||"").trim().toLocaleLowerCase("vi")===name.toLocaleLowerCase("vi")&&x.id!==id))return toast("Tên nguồn đã tồn tại");upsert("sourceNames",id,{name,description:String(data.description||"").trim(),note:String(data.note||"").trim(),updatedAt:now});}
  if(entity==="orderType"){const code=String(data.code||"").trim().toUpperCase();if(!code)return toast("Mã loại đơn là bắt buộc");if((state.orderTypes||[]).some(x=>String(x.code||"").trim().toUpperCase()===code&&x.id!==id))return toast("Mã loại đơn đã tồn tại");upsert("orderTypes",id,{code,color:normalizeColor(data.color)||orderTypeDefaultColor(code),description:String(data.description||"").trim(),note:String(data.note||"").trim(),updatedAt:now});}
  closeModal();save();}
...
function removeEntity(entity,id){let label,relations=0;if(entity==="customer"){label=customer(id)?.fullName;relations=linksForCustomer(id).length;}if(entity==="product"){label=product(id)?.cardName;relations=linksForProduct(id).length;}if(entity==="bank"){label=bank(id)?.name;relations=state.cardProducts.filter(x=>x.bankId===id).length;if(relations)return toast("Không thể xóa ngân hàng đang được dòng thẻ sử dụng");}if(entity==="orderType"){const item=(state.orderTypes||[]).find(x=>x.id===id);label=item?.code;relations=state.transactions.filter(x=>x.orderTypeCode===item?.code).length;}if(entity==="sourceName"){const item=(state.sourceNames||[]).find(x=>x.id===id);label=item?.name;relations=state.transactions.filter(x=>String(x.sourceName||"").trim()===item?.name).length;}if(!confirm(`Xóa “${label}”${relations?` và giữ nguyên ${relations} đơn đang tham chiếu`:""}?`))return;if(entity==="customer"){state.customers=state.customers.filter(x=>x.id!==id);state.customerCards=state.customerCards.filter(x=>x.customerId!==id);}if(entity==="product"){state.cardProducts=state.cardProducts.filter(x=>x.id!==id);state.customerCards=state.customerCards.filter(x=>x.cardProductId!==id);}if(entity==="bank")state.banks=state.banks.filter(x=>x.id!==id);if(entity==="orderType")state.orderTypes=(state.orderTypes||[]).filter(x=>x.id!==id);if(entity==="sourceName")state.sourceNames=(state.sourceNames||[]).filter(x=>x.id!==id);save("Đã xóa an toàn");}
