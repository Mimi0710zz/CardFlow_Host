...
function renderCustomers(){
  ...
  $("#view-customers").innerHTML=`<div class="panel"><div class="section-title"><h2>Khách hàng</h2></div><div class="toolbar toolbar-zoned customer-toolbar"><div class="toolbar-left"><input class="search" data-filter="customers.q" placeholder="Tìm Mã KH, tên, điện thoại, email, CardID, ngân hàng, tên thẻ..." value="${esc(f.q||"")}"></div><div class="toolbar-center">${assignedProductFilterSelect("customers.cardId","CardID",f.cardId)}${filterSelect("customers.bank","Ngân hàng",state.banks,f.bank,x=>x.name)}${plainFilter("customers.form","Hình thức",CARD_FORMS.map(x=>`${x}|${x}`),f.form)}${plainFilter("customers.ownership","Loại thẻ",OWNERSHIP_TYPES.map(x=>`${x}|${ownershipTypeLabel(x)}`),f.ownership,true)}${plainFilter("customers.brand","Tổ chức thẻ Quốc tế",cardBrandValues().map(x=>`${x}|${x}`),f.brand)}</div><div class="toolbar-right"><button class="primary" data-add="customer">Thêm</button><button data-edit-selected="customer">Tùy chỉnh</button><button class="danger" data-delete-selected="customer">Xóa</button></div></div>${entityTable(["Mã KH","Họ tên","Số điện thoại","Email","Số thẻ","Tổng hạn mức","Ngân hàng","Ghi chú"],rows,"customer")}</div>`;
}
...
function renderCards(){
  ...
  const rows=items.map(p=>{const links=linksForProduct(p.id);return `<tr data-id="${p.id}">${cell("Card ID",`<strong>${esc(p.cardId)}</strong>`)}${cell("Loại thẻ",ownershipTypeLabel(p.ownershipType))}${cell("Ngân hàng",esc(bank(p.bankId)?.name||"—"))}${cell("Tên thẻ",esc(p.cardName))}${cell("Hạng thẻ",esc(p.cardRank))}${cell("Tổ chức thẻ Quốc tế",esc(p.cardBrand||p.network||"—"))}${cell("Hình thức thẻ",esc(p.cardForm||"—"))}${cell("Số khách sở hữu",links.length)}${cell("Ghi chú",esc(p.notes||"—"))}</tr>`;});
  $("#view-cards").innerHTML=`<div class="panel"><div class="section-title"><h2>Dòng thẻ</h2></div><div class="toolbar toolbar-zoned card-toolbar"><div class="toolbar-left"><input class="search" data-filter="cards.q" placeholder="Tìm Card ID, ngân hàng, hạng thẻ..." value="${esc(f.q||"")}"></div><div class="toolbar-center">${filterSelect("cards.bank","Ngân hàng",state.banks,f.bank,x=>x.name)}${plainFilter("cards.rank","Hạng thẻ",CARD_RANKS.map(x=>`${x}|${x}`),f.rank,true)}${plainFilter("cards.ownership","Loại thẻ",OWNERSHIP_TYPES.map(x=>`${x}|${ownershipTypeLabel(x)}`),f.ownership)}${plainFilter("cards.brand","Tổ chức thẻ Quốc tế",cardBrandValues().map(x=>`${x}|${x}`),f.brand)}${plainFilter("cards.form","Hình thức thẻ",CARD_FORMS.map(x=>`${x}|${x}`),f.form)}</div><div class="toolbar-right"><button class="primary" data-add="product">Thêm</button><button data-edit-selected="product">Tùy chỉnh</button><button class="danger" data-delete-selected="product">Xóa</button></div></div>${entityTable(["Card ID","Loại thẻ","Ngân hàng","Tên thẻ","Hạng thẻ","Tổ chức thẻ Quốc tế","Hình thức thẻ","Số khách sở hữu","Ghi chú"],rows,"product")}</div>`;
}
...
function hostGuideItems(){return [
  ...
  {icon:"credit-card",title:"Quản lý danh sách Thẻ",summary:"Cấu hình Card master trước khi gán cho khách hàng.",html:`<p>Tab <strong>Thẻ</strong> là nguồn dữ liệu master. Các trường gồm Card ID, Ngân hàng, Tên thẻ, Hạng thẻ, Loại thẻ, Tổ chức thẻ Quốc tế, Hình thức thẻ, Trạng thái và Ghi chú.</p><p><strong>Hạng thẻ:</strong> Classic/Standard, Gold, Platinum, Signature, Infinite/Back Card. <strong>Loại thẻ</strong> là Tín dụng hoặc Ghi nợ. <strong>Tổ chức thẻ Quốc tế</strong> gồm American Express, JCB, MasterCard, NAPAS, Union Pay và Visa.</p>`},
...
function openForm(entity,id=null,preset={}){const modal=$("#modal"),form=$("form",modal),body=$(".modal-body",modal);form.dataset.entity=entity;form.dataset.id=id||"";modal.classList.toggle("customer-editor-modal",entity==="customer");let item;
  ...
  if(entity==="product"){item=state.cardProducts.find(x=>x.id===id)||preset;$("h2",modal).textContent=id?"Tùy chỉnh dòng thẻ":"Thêm dòng thẻ";body.innerHTML=field("cardId","Card ID",item.cardId,"text",true)+selectField("bankId","Ngân hàng",bankOptions(item.bankId),true)+field("cardName","Tên thẻ",item.cardName,"text",true)+selectField("cardRank","Hạng thẻ",rankOptions(item.cardRank||"Classic/Standard"),true)+selectField("ownershipType","Loại thẻ",OWNERSHIP_TYPES.map(value=>`<option value="${value}" ${value===(item.ownershipType||"credit")?"selected":""}>${ownershipTypeLabel(value)}</option>`).join(""),true)+selectField("cardBrand","Tổ chức thẻ Quốc tế",textValueOptions(cardBrandValues(),item.cardBrand||item.network||""),true)+selectField("cardForm","Hình thức thẻ",textValueOptions(CARD_FORMS,item.cardForm||"Vật lý"),true)+selectField("status","Trạng thái",statusOptions(item.status))+noteField(item.notes);}
