...
function renderCustomers(){
  ...
  $("#view-customers").innerHTML=`<div class="panel"><div class="section-title"><h2>Khách hàng</h2></div><div class="toolbar customer-toolbar"><input class="search" data-filter="customers.q" placeholder="Tìm Mã KH, tên, điện thoại, email, CardID, ngân hàng, tên thẻ..." value="${esc(f.q||"")}">${assignedProductFilterSelect("customers.cardId","CardID",f.cardId)}${filterSelect("customers.bank","Ngân hàng",state.banks,f.bank,x=>x.name)}${plainFilter("customers.form","Hình thức",CARD_FORMS.map(x=>`${x}|${x}`),f.form)}${plainFilter("customers.ownership","Loại thẻ",OWNERSHIP_TYPES.map(x=>`${x}|${ownershipTypeLabel(x)}`),f.ownership,true)}${plainFilter("customers.brand","Tổ chức thẻ Quốc tế",cardBrandValues().map(x=>`${x}|${x}`),f.brand)}<button class="primary" data-add="customer">Thêm</button><button data-edit-selected="customer">Tùy chỉnh</button><button class="danger" data-delete-selected="customer">Xóa</button></div>${entityTable(["Mã KH","Họ tên","Số điện thoại","Email","Số thẻ","Tổng hạn mức","Ngân hàng","Ghi chú"],rows,"customer")}</div>`;
}
...
function renderCards(){
  ...
  const rows=items.map(p=>{const links=linksForProduct(p.id);return `<tr data-id="${p.id}">${cell("Card ID",`<strong>${esc(p.cardId)}</strong>`)}${cell("Loại thẻ",ownershipTypeLabel(p.ownershipType))}${cell("Ngân hàng",esc(bank(p.bankId)?.name||"—"))}${cell("Tên thẻ",esc(p.cardName))}${cell("Hạng thẻ",esc(p.cardRank))}${cell("Tổ chức thẻ Quốc tế",esc(p.cardBrand||p.network||"—"))}${cell("Hình thức thẻ",esc(p.cardForm||"—"))}${cell("Số khách sở hữu",links.length)}</tr>`;});
  $("#view-cards").innerHTML=`<div class="panel"><div class="section-title"><h2>Dòng thẻ</h2></div><div class="toolbar card-toolbar"><input class="search" data-filter="cards.q" placeholder="Tìm Card ID, ngân hàng, hạng thẻ..." value="${esc(f.q||"")}">${filterSelect("cards.bank","Ngân hàng",state.banks,f.bank,x=>x.name)}${plainFilter("cards.rank","Hạng thẻ",CARD_RANKS.map(x=>`${x}|${x}`),f.rank,true)}${plainFilter("cards.ownership","Loại thẻ",OWNERSHIP_TYPES.map(x=>`${x}|${ownershipTypeLabel(x)}`),f.ownership)}${plainFilter("cards.brand","Tổ chức thẻ Quốc tế",cardBrandValues().map(x=>`${x}|${x}`),f.brand)}${plainFilter("cards.form","Hình thức thẻ",CARD_FORMS.map(x=>`${x}|${x}`),f.form)}<button class="primary" data-add="product">Thêm</button><button data-edit-selected="product">Tùy chỉnh</button><button class="danger" data-delete-selected="product">Xóa</button></div>${entityTable(["Card ID","Loại thẻ","Ngân hàng","Tên thẻ","Hạng thẻ","Tổ chức thẻ Quốc tế","Hình thức thẻ","Số khách sở hữu"],rows,"product")}</div>`;
}
