import {LocalRepository,uuid} from "./services/local-repository.js?v=20260831-cardmasterv7";
...
import {CARD_BRANDS,CARD_RANKS,OWNERSHIP_TYPES,normalizeCardBrand,normalizeCardRank,normalizeOwnershipType} from "./services/card-types.js?v=20260831-cardmasterv7";
...
function effectiveLimitDisplay(analysis){return analysis.inconsistencies.length?`<span class="limit-warning" title="Nhóm hạn mức chung có giá trị không đồng nhất">⚠ Cần kiểm tra</span>`:formatMoney(analysis.total);}
...
function renderDashboard(){
  const active=state.customerCards.filter(x=>x.status==="active"),limitAnalysis=effectiveLimit(active),attributed=attributedEffectiveLimits(limitAnalysis);
  const byBank=state.banks.map(b=>{const links=active.filter(l=>product(l.cardProductId)?.bankId===b.id),owned=[...attributed.entries()].filter(([productId])=>product(productId)?.bankId===b.id).map(([,value])=>value);return {name:b.name,count:links.length,analysis:{total:sum(owned,x=>x.total),inconsistencies:owned.flatMap(x=>x.inconsistencies)}}}).filter(x=>x.count).sort((a,b)=>b.analysis.total-a.analysis.total);
  ...
  const topCustomers=state.customers.map(c=>{const links=linksForCustomer(c.id),analysis=effectiveLimit(links);return {id:c.id,name:c.fullName,count:links.length,analysis}}).sort((a,b)=>b.count-a.count||b.analysis.total-a.analysis.total).slice(0,5);
  ...
  $("#view-dashboard").innerHTML=`${limitInconsistencyWarning(limitAnalysis)}<div class="kpis"><div class="kpi blue"><small>Tổng khách hàng</small><strong>${state.customers.length}</strong></div><div class="kpi teal"><small>Tổng thẻ đang quản lý</small><strong>${active.length}</strong></div><div class="kpi amber"><small>Tổng hạn mức</small><strong>${effectiveLimitDisplay(limitAnalysis)}</strong></div><div class="kpi indigo"><small>Số dòng thẻ</small><strong>${state.cardProducts.length}</strong></div></div>
  <div class="grid-2"><div class="panel"><h2>Thống kê theo ngân hàng</h2>${simpleTable(["Ngân hàng","Số thẻ","Tổng hạn mức"],byBank.map(x=>[x.name,x.count,effectiveLimitDisplay(x.analysis)]))}</div><div class="panel"><h2>Top dòng thẻ</h2>${simpleTable(["Dòng thẻ","Khách sở hữu"],topProducts.map(x=>[x.name,x.count]))}</div><div class="panel"><h2>Khách hàng có nhiều thẻ nhất</h2>${simpleTable(["Khách hàng","Số thẻ","Tổng hạn mức"],topCustomers.map(x=>[`<a href="#" data-open-customer="${x.id}">${esc(x.name)}</a>`,x.count,effectiveLimitDisplay(x.analysis)]))}</div><div class="panel"><h2>Ngày sắp tới</h2><div class="upcoming">${upcoming.map((x,i)=>`<div><span>${i===0?"Hôm nay":`${x.range} ngày tới`}</span><strong>${x.items.length} thẻ</strong></div>`).join("")}</div></div></div>`;
...
function renderCustomers(){
  const f=filters.customers||{}, q=normalize(f.q), bankFilter=f.bank||"",cardIdFilter=f.cardId||"",formFilter=f.form||"",brand=f.brand||"";
  let items=state.customers.filter(c=>{const links=linksForCustomer(c.id),products=links.map(x=>product(x.cardProductId)).filter(Boolean);const hay=[c.customerCode,c.fullName,c.phone,c.email,...products.flatMap(p=>[p.cardId,p.cardName,p.cardRank,p.ownershipType,p.cardBrand,p.network,p.cardForm,bank(p.bankId)?.name])].map(normalize).join(" ");return (!q||hay.includes(q))&&(!cardIdFilter||products.some(p=>p.id===cardIdFilter))&&(!bankFilter||products.some(p=>p.bankId===bankFilter))&&(!formFilter||products.some(p=>p.cardForm===formFilter))&&(!brand||products.some(p=>(p.cardBrand||p.network)===brand));});
  ...
  $("#view-customers").innerHTML=`<div class="panel"><div class="section-title"><h2>Khách hàng</h2></div><div class="toolbar customer-toolbar"><input class="search" data-filter="customers.q" placeholder="Tìm khách hàng hoặc thẻ..." value="${esc(f.q||"")}">${productFilterSelect("customers.cardId","Card ID",f.cardId)}${filterSelect("customers.bank","Ngân hàng",state.banks,f.bank,x=>x.name)}${plainFilter("customers.form","Hình thức",CARD_FORMS.map(x=>`${x}|${x}`),f.form)}${plainFilter("customers.brand","Loại thẻ",cardBrandValues().map(x=>`${x}|${x}`),f.brand)}<button class="primary" data-add="customer">Thêm</button><button data-edit-selected="customer">Tùy chỉnh</button><button class="danger" data-delete-selected="customer">Xóa</button></div>${entityTable(["Mã KH","Họ tên","Số điện thoại","Email","Số thẻ","Tổng hạn mức","Ngân hàng","Ghi chú"],rows,"customer")}</div>`;
}
...
function responsiveOwnedCards(customerItem,links){
  ...
  const cardItems=cards.map(({link,card})=>`<details class="owned-card-item"><summary><span><strong>${esc(card.cardId||"—")}</strong><small>${esc(card.cardRank)} · ${formatMoney(link.creditLimit)}</small></span>${badge(link.status)}<span class="owned-card-chevron">${icon("chevron")}</span></summary><div class="owned-card-detail">${info("Card ID",card.cardId)}${info("Ngân hàng",bank(card.bankId)?.name)}${info("Hạng thẻ",card.cardRank)}${info("Loại thẻ",card.cardBrand||card.network)}${info("Loại sở hữu",ownershipTypeLabel(card.ownershipType))}${info("Hình thức",card.cardForm||link.cardForm)}${info("Hạn mức",formatMoney(link.creditLimit))}${info("Sao kê",formatDay(link.statementDay))}${info("Đến hạn",formatDay(link.paymentDueDay))}${info("Chung hạn mức",link.sharedLimitCardIds?.length?`${link.sharedLimitCardIds.length+1} thẻ cùng nhóm`:"Không")}${info("Trạng thái",responsiveStatusLabel(link.status))}</div></details>`).join("");
...
function renderCards(){
  const allAnalysis=effectiveLimit(state.customerCards),attributed=attributedEffectiveLimits(allAnalysis);
  const f=filters.cards||{}, q=normalize(f.q);let items=state.cardProducts.filter(p=>{const links=linksForProduct(p.id),owners=links.map(x=>customer(x.customerId)).filter(Boolean);const hay=[p.cardId,bank(p.bankId)?.name,p.cardName,p.cardRank,p.ownershipType,p.cardBrand,p.network,p.cardForm,...owners.map(x=>x.fullName)].map(normalize).join(" ");return (!q||hay.includes(q))&&(!f.bank||p.bankId===f.bank)&&(!f.status||p.status===f.status)&&(!f.brand||(p.cardBrand||p.network)===f.brand)&&(!f.form||p.cardForm===f.form);});
  items.sort((a,b)=>compareCards(a,b,x=>bank(x.bankId)?.name||""));
  const rows=items.map(p=>{const links=linksForProduct(p.id),analysis=attributed.get(p.id)||{total:0,inconsistencies:[]};return `<tr data-id="${p.id}">${cell("Card ID",`<strong>${esc(p.cardId)}</strong>`)}${cell("Ngân hàng",esc(bank(p.bankId)?.name||"—"))}${cell("Tên thẻ",`<strong>${esc(p.cardName)}</strong>`)}${cell("Hạng thẻ",esc(p.cardRank))}${cell("Loại sở hữu",ownershipTypeLabel(p.ownershipType))}${cell("Loại thẻ",esc(p.cardBrand||p.network||"—"))}${cell("Hình thức thẻ",esc(p.cardForm||"—"))}${cell("Số khách sở hữu",links.length)}${cell("Tổng hạn mức",effectiveLimitDisplay(analysis))}${cell("Hạn mức trung bình",analysis.inconsistencies.length?effectiveLimitDisplay(analysis):formatMoney(links.length?analysis.total/links.length:0))}${cell("Ghi chú",esc(p.notes||"—"))}</tr>`;});
  $("#view-cards").innerHTML=`<div class="panel"><div class="section-title"><h2>Dòng thẻ</h2></div><div class="toolbar"><input class="search" data-filter="cards.q" placeholder="Tìm Card ID, ngân hàng, hạng thẻ, loại thẻ, hình thức, khách sở hữu..." value="${esc(f.q||"")}">${filterSelect("cards.bank","Ngân hàng",state.banks,f.bank,x=>x.name)}${plainFilter("cards.status","Trạng thái",["active|Đang hoạt động","inactive|Ngừng hoạt động"],f.status)}${plainFilter("cards.brand","Loại thẻ",cardBrandValues().map(x=>`${x}|${x}`),f.brand)}${plainFilter("cards.form","Hình thức thẻ",CARD_FORMS.map(x=>`${x}|${x}`),f.form)}<button class="primary" data-add="product">Thêm</button><button data-edit-selected="product">Tùy chỉnh</button><button class="danger" data-delete-selected="product">Xóa</button></div>${entityTable(["Card ID","Ngân hàng","Tên thẻ","Hạng thẻ","Loại sở hữu","Loại thẻ","Hình thức thẻ","Số khách sở hữu","Tổng hạn mức","Hạn mức TB","Ghi chú"],rows,"product")}</div>`;
}
...
function hostGuideItems(){return [
  {icon:"play",title:"Bắt đầu sử dụng",summary:"Quy trình thiết lập Host theo đúng thứ tự.",html:`<ol class="guide-steps"><li><strong>Kết nối Google Drive.</strong></li><li>Vào tab <strong>Thẻ</strong> và cấu hình danh sách thẻ trước.</li><li>Vào tab <strong>Khách hàng</strong> để tạo khách hàng.</li><li>Gán các CardID đã cấu hình cho khách hàng.</li><li>Kiểm tra tab <strong>Tổng hợp</strong> và chi tiết khách hàng.</li></ol>`},
  {icon:"cloud",title:"Kết nối Google Drive",summary:"Đăng nhập và tải dữ liệu Host từ Drive.",html:`<p>Khi mở ứng dụng, bấm <strong>Kết nối Google Drive</strong>, chọn tài khoản Google đã được cấp quyền/tester và chấp thuận quyền cần thiết. Sau khi kết nối thành công, ứng dụng tải dữ liệu Host bằng cơ chế đồng bộ hiện có.</p><p>Các trạng thái gồm: <strong>Chưa kết nối</strong>, <strong>Đang đồng bộ</strong>, <strong>Đã đồng bộ</strong>, <strong>Có thay đổi chưa đồng bộ</strong> và <strong>Lỗi đồng bộ / xung đột</strong>.</p>`},
  {icon:"layout-dashboard",title:"Tab Tổng hợp",summary:"Theo dõi nhanh số liệu và các ngày quan trọng.",html:`<p>Dashboard hiển thị <strong>Tổng khách hàng</strong>, <strong>Tổng thẻ đang quản lý</strong>, <strong>Tổng hạn mức</strong>, <strong>Số dòng thẻ</strong>, thống kê theo ngân hàng, Top dòng thẻ, khách hàng có nhiều thẻ nhất và Ngày sắp tới.</p><div class="help-callout"><strong>Cách tính hạn mức</strong><p>Các thẻ thuộc cùng một nhóm chung hạn mức chỉ được tính một lần. Nhóm có giá trị không đồng nhất sẽ được cảnh báo để kiểm tra.</p></div>`},
  {icon:"credit-card",title:"Quản lý danh sách Thẻ",summary:"Cấu hình Card master trước khi gán cho khách hàng.",html:`<p>Tab <strong>Thẻ</strong> là nguồn dữ liệu master. Các trường hiện có gồm Card ID, Ngân hàng, Tên thẻ, Hạng thẻ, Loại sở hữu, Loại thẻ, Hình thức thẻ, Trạng thái và Ghi chú.</p><p>Loại thẻ hỗ trợ: <strong>American Express, JCB, MasterCard, NAPAS, Union Pay, Visa</strong>. Loại sở hữu là Tín dụng hoặc Ghi nợ; Card ID phải duy nhất.</p>`},
  {icon:"users",title:"Thêm và quản lý Khách hàng",summary:"Tạo, xem, tùy chỉnh và xóa khách hàng.",html:`<p>Khách hàng gồm Họ tên, Số điện thoại, Email, Ngày sinh và Ghi chú; Mã KH được ứng dụng tự sinh và giữ nguyên khi chỉnh sửa.</p><p>Dùng <strong>Thêm</strong>, <strong>Tùy chỉnh</strong>, <strong>Xóa</strong>; nhấp đúp một dòng hoặc dùng menu chuột phải để mở chi tiết theo thao tác hiện có.</p>`},
  {icon:"link",title:"Gán thẻ cho Khách hàng",summary:"Quản lý nhiều CardID ngay trong form Khách hàng.",html:`<p>Trong Thêm/Tùy chỉnh khách hàng, khu vực <strong>Thẻ của khách hàng</strong> gồm Thẻ/CardID, Hình thức thẻ, Hạn mức, Ngày sao kê, Hạn thanh toán và Chung hạn mức.</p><p>Dropdown lấy từ Card master. Bấm <strong>+</strong> để thêm dòng; bấm <strong>×</strong> để bỏ một dòng trước khi lưu. Một khách hàng có thể sở hữu nhiều thẻ.</p>`},
  {icon:"link",title:"Chung hạn mức",summary:"Liên kết nhiều CardID vào cùng một hạn mức thực tế.",html:`<p>Nhấp vào trường <strong>Chung hạn mức</strong>, nhập một phần CardID rồi chọn kết quả để thêm chip. Có thể tiếp tục nhập để thêm nhiều thẻ; CardID đã chọn không còn trong gợi ý. Bấm <strong>×</strong> trên chip để bỏ riêng CardID đó.</p><div class="help-callout example"><strong>Ví dụ</strong><p>Card A: 30.000.000 đ; Card B: 30.000.000 đ; A và B chung hạn mức. Tổng hiệu lực của nhóm là <strong>30.000.000 đ</strong>, không phải 60.000.000 đ. Nhóm từ ba thẻ trở lên cũng chỉ được tính một lần.</p></div>`},
  {icon:"search",title:"Tìm kiếm & bộ lọc",summary:"Thu hẹp nhanh danh sách Khách hàng và Thẻ.",html:`<p>Tìm kiếm Khách hàng hỗ trợ Mã KH, Họ tên, Số điện thoại, Email, Ngân hàng, Hạng thẻ và Card ID. Các bộ lọc gồm Card ID, Ngân hàng, Hình thức và Loại thẻ.</p><p>Danh sách Thẻ có tìm kiếm và các bộ lọc Ngân hàng, Trạng thái, Loại thẻ, Hình thức thẻ. Tùy chọn đặc biệt “Tất cả” luôn nằm ở đầu.</p>`},
  {icon:"edit",title:"Thêm / Tùy chỉnh / Xóa dữ liệu",summary:"Các thao tác CRUD và thứ tự lưu an toàn.",html:`<p><strong>Thêm</strong> tạo bản ghi mới, <strong>Tùy chỉnh</strong> cập nhật dòng đã chọn và <strong>Xóa</strong> loại bản ghi sau khi xác nhận.</p><p>Khi bấm Lưu, dữ liệu được ghi local trước rồi auto-sync Google Drive. Người dùng thường không cần bấm <strong>Đồng bộ ngay</strong> sau mỗi lần lưu; nút này vẫn là thao tác thủ công dự phòng.</p>`},
  {icon:"refresh",title:"Đồng bộ dữ liệu Google Drive",summary:"Local-first, tự đồng bộ sau thay đổi bền vững.",html:`<div class="sync-flow"><span>Thay đổi dữ liệu</span><b>→</b><span>Lưu local</span><b>→</b><span>Tự đồng bộ Drive</span><b>→</b><span>Cập nhật trạng thái</span></div><p>Ứng dụng lưu local trước và xếp hàng đồng bộ sau thay đổi. Nếu Drive bị ngắt kết nối, dữ liệu local vẫn được giữ ở trạng thái chờ đồng bộ. <strong>Đồng bộ ngay</strong> vẫn có thể dùng làm phương án thủ công.</p>`},
  {icon:"devices",title:"Sử dụng trên nhiều thiết bị",summary:"Tải cùng dữ liệu Host bằng cùng tài khoản được phép.",html:`<p>Trên thiết bị khác, mở Host web app, kết nối cùng tài khoản Google đã được phép và chờ cơ chế đồng bộ tải dữ liệu Host từ Drive trước khi chỉnh sửa.</p><div class="help-callout"><strong>Lưu ý</strong><p>Ứng dụng không phải hệ thống cộng tác thời gian thực. Hãy kiểm tra trạng thái đồng bộ trước khi chuyển thiết bị.</p></div>`},
  {icon:"database",title:"Sao lưu & khôi phục dữ liệu",summary:"Backup Drive tự động và bản sao JSON thủ công.",html:`<p>Trước khi cập nhật một file Drive đã tồn tại, ứng dụng tạo bản sao dữ liệu từ Drive với tên dạng <code>cardflow-host-backup-YYYY-MM-DD.json</code>. Tab Hệ thống có nút <strong>Xuất bản sao JSON</strong> để tải bản sao local.</p><p>Host hiện không có nút khôi phục tự động trong UI. Không nên sửa trực tiếp file JSON nếu chưa hiểu cấu trúc dữ liệu.</p>`},
  {icon:"table-properties",title:"Danh mục",summary:"Quản lý danh sách Ngân hàng dùng chung.",html:`<p>Tab <strong>Danh mục</strong> hiện quản lý Mã ngân hàng và Tên ngân hàng. Hãy cấu hình ngân hàng trước khi chọn ngân hàng trong Card master. Ngân hàng đang được thẻ sử dụng không thể xóa.</p>`},
  {icon:"settings",title:"Hệ thống",summary:"Nhập Excel, xuất JSON và xem thông tin lưu trữ.",html:`<p>Tab <strong>Hệ thống</strong> cho phép tải template, nhập dữ liệu Excel, xem kết quả nhập, xuất bản sao JSON và xem tên namespace local/file Drive đang sử dụng.</p><p>Không chia sẻ token OAuth hoặc thông tin đăng nhập. Dùng các điều khiển trong ứng dụng cho thao tác dữ liệu thông thường.</p>`},
  {icon:"info",title:"Lưu ý khi sử dụng",summary:"Các nguyên tắc giúp dữ liệu Host nhất quán.",html:`<ul><li>Cấu hình Card master trước khi tạo quan hệ thẻ của khách hàng.</li><li>Không tạo Card ID trùng.</li><li>Các thẻ chung hạn mức phải đại diện cho cùng một hạn mức thực tế.</li><li>Kiểm tra trạng thái Drive trước khi đóng ứng dụng sau thay đổi quan trọng.</li><li>Không chỉnh file Drive JSON thủ công nếu không thật sự cần thiết và hiểu rõ dữ liệu.</li><li>Dùng các nút của ứng dụng cho CRUD thông thường.</li></ul>`}
];}
function renderAbout(){
  const guides=hostGuideItems();
  $("#view-about").innerHTML=`<div class="help-center host-help"><section class="help-section"><div class="help-section-head"><span class="help-section-icon">${icon("info")}</span><div><h2>Giới thiệu</h2><p>Thông tin về CardFlow Host và cách dữ liệu được quản lý.</p></div></div><div class="about-layout"><article class="panel about-card"><h2>QUẢN LÝ THẺ - HOST</h2><p>Ứng dụng quản lý tập trung khách hàng và danh sách thẻ đang quản lý.</p><div class="about-features"><span>Quản lý Card master</span><span>Quản lý khách hàng và thẻ</span><span>Local-first</span><span>Auto-sync Google Drive</span><span>Hỗ trợ nhiều thiết bị</span><span>Dữ liệu độc lập với Client</span></div></article><article class="panel about-card"><h2>Thông tin ứng dụng</h2><p>Dữ liệu Host được lưu local trước, sau đó đồng bộ vào Google Drive khi đã kết nối. Khi dùng cùng tài khoản Google được phép trên thiết bị khác, ứng dụng có thể tải lại dữ liệu Host thông qua cơ chế sync hiện có.</p><p><strong>Phiên bản:</strong> CardFlow Host Web App · Schema V2.</p><p>Ứng dụng không lưu số thẻ đầy đủ, CVV/CVC, OTP, PIN hoặc thông tin đăng nhập ngân hàng.</p></article></div></section><section class="help-section"><div class="help-section-head"><span class="help-section-icon">${icon("circle-help")}</span><div><h2>Hướng dẫn sử dụng</h2><p>Chọn từng mục để mở hoặc thu gọn nội dung.</p></div></div><div class="guide-list">${guides.map((guide,index)=>`<details class="guide-item" ${index===0?"open":""}><summary><span class="guide-icon">${icon(guide.icon)}</span><span class="guide-heading"><strong>${esc(guide.title)}</strong><small>${esc(guide.summary)}</small></span><span class="guide-chevron">${icon("chevron")}</span></summary><div class="guide-detail"><div class="guide-detail-inner">${guide.html}</div></div></details>`).join("")}</div></section></div>`;
}
...
function openForm(entity,id=null,preset={}){const modal=$("#modal"),form=$("form",modal),body=$(".modal-body",modal);form.dataset.entity=entity;form.dataset.id=id||"";modal.classList.toggle("customer-editor-modal",entity==="customer");let item;
  ...
  if(entity==="product"){item=state.cardProducts.find(x=>x.id===id)||preset;$("h2",modal).textContent=id?"Tùy chỉnh dòng thẻ":"Thêm dòng thẻ";body.innerHTML=field("cardId","Card ID",item.cardId,"text",true)+selectField("bankId","Ngân hàng",bankOptions(item.bankId),true)+field("cardName","Tên thẻ",item.cardName,"text",true)+selectField("cardRank","Hạng thẻ",rankOptions(item.cardRank||"Standard"),true)+selectField("ownershipType","Loại sở hữu",OWNERSHIP_TYPES.map(value=>`<option value="${value}" ${value===(item.ownershipType||"credit")?"selected":""}>${ownershipTypeLabel(value)}</option>`).join(""),true)+selectField("cardBrand","Loại thẻ",textValueOptions(cardBrandValues(),item.cardBrand||item.network||""),true)+selectField("cardForm","Hình thức thẻ",textValueOptions(CARD_FORMS,item.cardForm||"Vật lý"),true)+selectField("status","Trạng thái",statusOptions(item.status))+noteField(item.notes);}
...
function submitForm(event){event.preventDefault();const form=event.currentTarget,entity=form.dataset.entity,id=form.dataset.id,data=Object.fromEntries(new FormData(form));const now=new Date().toISOString();
  ...
  if(entity==="product"){if(state.cardProducts.some(x=>x.cardId.toLowerCase()===data.cardId.trim().toLowerCase()&&x.id!==id))return toast("Card ID đã tồn tại");const ownershipType=normalizeOwnershipType(data.ownershipType);upsert("cardProducts",id,{...data,cardId:data.cardId.trim(),cardName:data.cardName.trim(),cardRank:normalizeCardRank(data.cardRank),ownershipType,cardType:ownershipType,cardBrand:normalizeCardBrand(data.cardBrand),cardForm:data.cardForm.trim(),updatedAt:now});}
...
function openCustomerDetail(id){
  ...
  const rows=displayRows.map(({link:l,card:p,bankName})=>`<tr data-product-link="${p.id}"><td data-label="Card ID"><strong>${esc(p.cardId||"—")}</strong></td>${cell("Ngân hàng",esc(bankName||"—"))}${cell("Hạng thẻ",esc(p.cardRank))}${cell("Loại thẻ",esc(p.cardBrand||p.network||l.cardBrand||"—"))}${cell("Loại sở hữu",ownershipTypeLabel(p.ownershipType))}${cell("Hạn mức",formatMoney(l.creditLimit))}${cell("Sao kê",formatDay(l.statementDay))}${cell("Đến hạn",formatDay(l.paymentDueDay))}${cell("Chung hạn mức",l.sharedLimitCardIds?.length?`${l.sharedLimitCardIds.length+1} thẻ cùng nhóm`:"Không")}${cell("Trạng thái",badge(l.status))}</tr>`);
  openDetailModal("",`<div class="detail-info">${info("Mã KH",c.customerCode)}${info("Họ tên",c.fullName)}${info("Số điện thoại",c.phone)}${info("Email",c.email)}${info("Ngày sinh",formatDate(c.dateOfBirth))}${info("Ghi chú",c.notes)}</div>${limitInconsistencyWarning(analysis)}<div class="kpis" style="margin-top:14px"><div class="kpi blue"><small>Số thẻ</small><strong>${links.length}</strong></div><div class="kpi teal"><small>Tổng hạn mức</small><strong>${effectiveLimitDisplay(analysis)}</strong></div><div class="kpi amber"><small>Số ngân hàng</small><strong>${banks.size}</strong></div><div class="kpi red"><small>Không hoạt động</small><strong>${links.filter(x=>x.status!=="active").length}</strong></div></div><div class="panel" style="margin-top:14px"><div class="section-title"><h2>Danh sách thẻ đang sở hữu</h2><button class="primary" data-add-link>Thêm thẻ</button></div>${entityTable(["Card ID","Ngân hàng","Hạng thẻ","Loại thẻ","Loại sở hữu","Hạn mức","Sao kê","Đến hạn","Chung hạn mức","Trạng thái"],rows,"detail-links")}</div>`);$("h2",$("#detailModal")).innerHTML=`<span>Khách hàng:</span> <strong class="customer-name-accent">${esc(c.fullName.toLocaleUpperCase("vi"))}</strong>`;$('[data-add-link]').onclick=()=>{closeDetail();openForm("link",null,{customerId:id});};$$('[data-product-link]').forEach(x=>x.onclick=()=>openProductDetail(x.dataset.productLink));
}
function openProductDetail(id){
  const p=product(id);if(!p)return;const links=linksForProduct(id),analysis=attributedEffectiveLimits(effectiveLimit(state.customerCards)).get(id)||{total:0,inconsistencies:[]},rows=links.map(l=>{const c=customer(l.customerId);return `<tr data-customer-link="${c?.id||""}">${cell("Mã KH",esc(c?.customerCode||"—"))}${cell("Họ tên",esc(c?.fullName||"—"))}${cell("Số điện thoại",esc(c?.phone||"—"))}${cell("Hạn mức",formatMoney(l.creditLimit))}${cell("Ngày sao kê",formatDay(l.statementDay))}${cell("Ngày đến hạn",formatDay(l.paymentDueDay))}${cell("Trạng thái",badge(l.status))}</tr>`;});
  openDetailModal(`Dòng thẻ: ${p.cardName}`,`<div class="detail-info">${info("Card ID",p.cardId)}${info("Ngân hàng",bank(p.bankId)?.name)}${info("Tên thẻ",p.cardName)}${info("Hạng thẻ",p.cardRank)}${info("Loại sở hữu",ownershipTypeLabel(p.ownershipType))}${info("Loại thẻ",p.cardBrand||p.network)}${info("Hình thức thẻ",p.cardForm)}${info("Ghi chú",p.notes)}</div><div class="kpis" style="margin-top:14px"><div class="kpi blue"><small>Số khách sở hữu</small><strong>${links.length}</strong></div><div class="kpi teal"><small>Tổng hạn mức</small><strong>${effectiveLimitDisplay(analysis)}</strong></div><div class="kpi indigo"><small>Hạn mức trung bình</small><strong>${analysis.inconsistencies.length?effectiveLimitDisplay(analysis):formatMoney(links.length?analysis.total/links.length:0)}</strong></div></div><div class="panel" style="margin-top:14px"><h2>Danh sách khách đang sở hữu</h2>${entityTable(["Mã KH","Họ tên","Số điện thoại","Hạn mức","Ngày sao kê","Ngày đến hạn","Trạng thái"],rows,"detail-owners")}</div>`);$$('[data-customer-link]').forEach(x=>x.onclick=()=>openCustomerDetail(x.dataset.customerLink));
}
...
function downloadTemplate(){if(!window.XLSX)return toast("Chưa tải được thư viện Excel");const wb=XLSX.utils.book_new();const sheets={"01_KhachHang":[["MaKH","HoTen","SoDienThoai","Email","NgaySinh","GhiChu"]],"02_DongThe":[["CardID","NganHang","TenThe","HangThe","LoaiSoHuu","LoaiThe","HinhThucThe","GhiChu"]],"03_TheKhachHang":[["MaKH","CardID","HanMuc","NgaySaoKe","NgayDenHan","NgayMoThe","NgayHetHan","BonSoCuoi","TrangThai","GhiChu"]],"04_HuongDan":[["HƯỚNG DẪN NHẬP DỮ LIỆU HOST"],["MaKH và CardID phải duy nhất."],["HangThe: Classic, Standard, Gold, Platinum, Infinite hoặc Black Card."],["LoaiSoHuu: Credit hoặc Debit."],["03_TheKhachHang chỉ tham chiếu MaKH và CardID đã có."],["Ngày dùng DD-MM-YYYY hoặc YYYY-MM-DD. Ngày sao kê/đến hạn từ 1 đến 31."],["Không nhập số thẻ đầy đủ, CVV/CVC, OTP hoặc PIN."]]};Object.entries(sheets).forEach(([name,data])=>XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet(data),name));XLSX.writeFile(wb,"CardFlow_Host_Import_Template.xlsx");}
