...
function renderCustomers(){
  ...
  $("#view-customers").innerHTML=`<div class="panel"><div class="section-title"><h2>Khách hàng</h2></div>${compactEntityToolbar("customers","customer","Tìm Mã KH, tên, điện thoại, email, CardID, ngân hàng, tên thẻ...",f,filterFields)}${entityTable(["Mã KH","Họ tên","Số điện thoại","Email","Số thẻ","Tổng hạn mức","Ngân hàng","Ghi chú"],rows,"customer")}</div>`;
}
...
function renderCards(){
  ...
  $("#view-cards").innerHTML=`<div class="panel"><div class="section-title"><h2>Thẻ ngân hàng</h2></div>${compactEntityToolbar("cards","product","Tìm Card ID, ngân hàng, hạng thẻ...",f,filterFields)}${entityTable(["Card ID","Loại thẻ","Ngân hàng","Tên thẻ","Hạng thẻ","Phôi","Hình thức thẻ","Hình thức hoàn","Số khách sở hữu","Ghi chú"],rows,"product")}</div>`;
}
function compactEntityToolbar(group,entity,placeholder,current,fields){
  ...
  return `<div class="toolbar feature-main-toolbar entity-main-toolbar"><input class="search" data-filter="${group}.q" placeholder="${esc(placeholder)}" value="${esc(current.q||"")}"><div class="toolbar-spacer"></div><button type="button" class="filter-trigger ${count?"active":""}" data-compact-trigger="${group}">${icon("filter")}<span>Bộ lọc</span><b data-filter-count ${count?"":"hidden"}>${count||""}</b></button><button class="primary" data-add="${entity}">Thêm</button><button data-edit-selected="${entity}">Tùy chỉnh</button><button class="danger" data-delete-selected="${entity}">Xóa</button></div><div class="filter-panel compact-filter-panel" data-compact-panel="${group}" hidden>${prepared}<button type="button" class="primary" data-compact-apply="${group}">Áp dụng</button><button type="button" class="secondary-btn" data-compact-clear="${group}">Xóa lọc</button></div>`;
}
...
function bindTables(){
  ...
  $$('[data-compact-trigger]').forEach(button=>button.onclick=()=>{const panel=$(`[data-compact-panel="${button.dataset.compactTrigger}"]`),willOpen=panel?.hidden;$$('[data-compact-panel]').forEach(item=>item.hidden=true);if(panel)panel.hidden=!willOpen;button.classList.toggle("active",Boolean(willOpen)||Boolean(button.querySelector('[data-filter-count]:not([hidden])')));});
