import {calculateProgress,isProgramEligible} from "./cashback-progress.js?v=20260901-drive-connect-hotfix-v82";
...
export function recommendOrders(state,{amount=0,mccCategoryId="",transactionMethod="",referenceDate=new Date()}={}){return buildCoordinationRows(state,referenceDate).filter(x=>x.progress.valid&&x.progress.status==="incomplete"&&isProgramEligible(x.program,{date:new Date(referenceDate).toISOString().slice(0,10),mccCategoryId,transactionMethod})).map(x=>{const eligibleAmount=Number(amount)||0,incremental=Math.min(eligibleAmount*(Number(x.program.rate)||0)/100,x.program.maxCashbackUnlimited?Infinity:Math.max(0,(Number(x.program.maxCashback)||0)-x.progress.cashbackEarned)),remainingAfter=x.progress.remainingEligibleSpend==null?null:Math.max(0,x.progress.remainingEligibleSpend-eligibleAmount),wouldWinExclusive=x.program.exclusiveMode==="first_reached"&&remainingAfter===0;return {...x,eligibleAmount,incrementalCashback:incremental,remainingAfter,wouldWinExclusive,wouldLockExclusiveGroup:wouldWinExclusive};}).sort((a,b)=>Number(b.wouldWinExclusive)-Number(a.wouldWinExclusive)||Number(a.remainingAfter??Infinity)-Number(b.remainingAfter??Infinity)||b.incrementalCashback-a.incrementalCashback||a.progress.daysRemaining-b.progress.daysRemaining);}
