...
import {getProgramPriority} from "./cashback-program.js?v=20260901-cashback-program-crud-v6";
...
export function canonicalize(input={}){
  ...
  const cashbackPrograms=(Array.isArray(input.cashbackPrograms)?input.cashbackPrograms:[]).filter(x=>x&&typeof x==="object"&&productIds.has(x.bankCardProductId||x.cardProductId)).map(x=>{const unlimited=x.maxCashbackUnlimited===true||x.maxCashbackMode==="unlimited",rate=Number(x.rate??x.cashbackRate)||0,max=unlimited?null:parseMoney(x.maxCashback??x.max),selected=uniqueTextArray(x.mccCategoryIds).filter(id=>mccIds.has(id)),mode=x.mccSelectionMode==="all"||x.allMcc===true?"all":x.mccSelectionMode==="selected"?"selected":selected.length?"selected":"all",excluded=uniqueTextArray(x.excludedMccCategoryIds).filter(id=>mccIds.has(id));return {id:x.id||uuid(),bankCardProductId:x.bankCardProductId||x.cardProductId,name:text(x.name),startDate:toStorageDate(x.startDate),endDate:toStorageDate(x.endDate),mccSelectionMode:mode,mccCategoryIds:mode==="selected"?selected:[],excludedMccCategoryIds:mode==="all"?excluded:[],allMcc:mode==="all",transactionMethod:text(x.transactionMethod||x.channel),rate,maxCashbackUnlimited:unlimited,maxCashback:max,eligibleTarget:unlimited?null:(parseMoney(x.eligibleTarget)||Math.round(max/(rate/100||1))),totalTarget:x.totalTarget==null||x.totalTarget===""?null:parseMoney(x.totalTarget),sharedCashbackGroup:text(x.sharedCashbackGroup||x.shared),priority:getProgramPriority(x),notes:text(x.notes),status:text(x.status)||"active",createdAt:x.createdAt||now,updatedAt:x.updatedAt||now};});
