...
import {getProgramPriority,normalizeProgramMccSelection} from "./cashback-program.js?v=20260901-program-mcc-selector-v1";
...
export function canonicalize(input={}){
  ...
  const cashbackPrograms=(Array.isArray(input.cashbackPrograms)?input.cashbackPrograms:[]).filter(x=>x&&typeof x==="object"&&productIds.has(x.bankCardProductId||x.cardProductId)).map(x=>{const unlimited=x.maxCashbackUnlimited===true||x.maxCashbackMode==="unlimited",selection=normalizeProgramMccSelection(x),eligibleMccCategoryIds=selection.eligibleMccCategoryIds.filter(id=>mccIds.has(id)),excludedMccCategoryIds=selection.excludedMccCategoryIds.filter(id=>mccIds.has(id));const rate=Number(x.rate??x.cashbackRate)||0,max=unlimited?null:parseMoney(x.maxCashback??x.max);return {id:x.id||uuid(),bankCardProductId:x.bankCardProductId||x.cardProductId,name:text(x.name),startDate:toStorageDate(x.startDate),endDate:toStorageDate(x.endDate),mccSelectionMode:selection.mccSelectionMode,eligibleMccCategoryIds,excludedMccCategoryIds,mccCategoryIds:eligibleMccCategoryIds,allMcc:selection.mccSelectionMode==="all",transactionMethod:text(x.transactionMethod||x.channel),rate,maxCashbackUnlimited:unlimited,maxCashback:max,eligibleTarget:unlimited?null:(parseMoney(x.eligibleTarget)||Math.round(max/(rate/100||1))),totalTarget:x.totalTarget==null||x.totalTarget===""?null:parseMoney(x.totalTarget),sharedCashbackGroup:text(x.sharedCashbackGroup||x.shared),priority:getProgramPriority(x),notes:text(x.notes),status:text(x.status)||"active",createdAt:x.createdAt||now,updatedAt:x.updatedAt||now};});
  const programIds=new Set(cashbackPrograms.map(x=>x.id));
  const linkIds=new Set(customerCards.map(x=>x.id));
  const transactions=(Array.isArray(input.transactions)?input.transactions:[]).filter(x=>x&&typeof x==="object"&&customerIds.has(x.customerId)&&linkIds.has(x.customerCardId||x.cardId)).map(x=>({id:x.id||uuid(),date:toStorageDate(x.date),customerId:x.customerId,customerCardId:x.customerCardId||x.cardId,amount:parseMoney(x.amount),mccCategoryId:mccIds.has(x.mccCategoryId)?x.mccCategoryId:"",transactionMethod:text(x.transactionMethod||x.channel),cashbackProgramId:text(x.cashbackProgramId),status:["completed","pending","cancelled"].includes(x.status)?x.status:"completed",notes:text(x.notes||x.note),createdAt:x.createdAt||now,updatedAt:x.updatedAt||now}));
