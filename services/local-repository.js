...
import {calculateTransactionAmounts,parsePercent} from "./transaction-calculations.js";
import {normalizeCardBrand,normalizeCardRank,normalizeOwnershipType} from "./card-types.js?v=20260831-cardranksv8";
import {getProgramPriority,normalizeCashbackConditions,normalizeCombineOperator} from "./cashback-program.js?v=20260905-cashback-conditions-v1";
import {normalizeExclusiveProgram} from "./cashback-exclusive.js";
import {normalizeCanonicalCashbackPrograms} from "./cashback-canonical.js?v=20260922-host-cashback-parity-v1";
...
export function canonicalize(input={}){
  ...
  const usedSourceNameIds=new Set(),seenSourceNames=new Set();
  const sourceNames=(Array.isArray(input.sourceNames)?input.sourceNames:[]).filter(x=>x&&typeof x==="object").map(x=>{const name=text(x.name||x.sourceName),key=name.toLocaleLowerCase("vi");if(!name||seenSourceNames.has(key))return null;seenSourceNames.add(key);let id=text(x.id);if(!id||usedSourceNameIds.has(id)){do{id=uuid();}while(usedSourceNameIds.has(id));}usedSourceNameIds.add(id);return {id,name,description:text(x.description),note:text(x.note??x.notes),createdAt:x.createdAt||now,updatedAt:x.updatedAt||now};}).filter(Boolean);
  (Array.isArray(input.transactions)?input.transactions:[]).forEach(tx=>{const name=text(tx?.sourceName),key=name.toLocaleLowerCase("vi");if(!name||seenSourceNames.has(key))return;seenSourceNames.add(key);let id=uuid();while(usedSourceNameIds.has(id))id=uuid();usedSourceNameIds.add(id);sourceNames.push({id,name,description:"",note:"",createdAt:now,updatedAt:now});});
  const mccIds=new Set(mccCategories.map(x=>x.id));
const cashbackPrograms=normalizeCanonicalCashbackPrograms((Array.isArray(input.cashbackPrograms)?input.cashbackPrograms:[]).filter(x=>x&&typeof x==="object"&&productIds.has(x.bankCardProductId||x.cardProductId)),{mccCategories}).map(x=>({...x,startDate:toStorageDate(x.startDate),endDate:toStorageDate(x.endDate),createdAt:x.createdAt||now,updatedAt:x.updatedAt||now}));
  const programIds=new Set(cashbackPrograms.map(x=>x.id));
  const linkIds=new Set(customerCards.map(x=>x.id));
  const paymentStatus=value=>["completed","pending","cancelled"].includes(value)?value:"pending";
  const transactions=(Array.isArray(input.transactions)?input.transactions:[]).filter(x=>x&&typeof x==="object"&&customerIds.has(x.customerId)&&linkIds.has(x.customerCardId||x.cardId)).map(x=>{const fee={amount:parseMoney(x.amount),orderFeePercent:parsePercent(x.orderFeePercent),orderFeeFixed:parseMoney(x.orderFeeFixed),sourceFeePercent:parsePercent(x.sourceFeePercent),sourceFeeFixed:parseMoney(x.sourceFeeFixed)},calculated=calculateTransactionAmounts(fee);return {id:x.id||uuid(),date:toStorageDate(x.date),timestamp:text(x.timestamp||x.dateTime||x.datetime),customerId:x.customerId,customerCardId:x.customerCardId||x.cardId,amount:fee.amount,mccCategoryId:mccIds.has(x.mccCategoryId)?x.mccCategoryId:"",mccCode:text(x.mccCode||x.mcc),orderTypeCode:text(x.orderTypeCode||x.orderType||x.type),transactionMethod:text(x.transactionMethod||x.channel),orderFeePercent:fee.orderFeePercent,orderFeeFixed:fee.orderFeeFixed,customerNetAmount:calculated.customerNetAmount,sourceFeePercent:fee.sourceFeePercent,sourceFeeFixed:fee.sourceFeeFixed,sourceNetAmount:calculated.sourceNetAmount,sourceName:text(x.sourceName),sourcePaymentStatus:paymentStatus(x.sourcePaymentStatus||x.receiptStatus),customerPaymentStatus:paymentStatus(x.customerPaymentStatus||x.payoutStatus),profit:calculated.profit,cashbackProgramId:text(x.cashbackProgramId),status:["completed","pending","cancelled"].includes(x.status)?x.status:"completed",notes:text(x.notes||x.note),createdAt:x.createdAt||now,updatedAt:x.updatedAt||now};});
  return {...base,...input,schemaVersion:3,deviceId:input.deviceId||base.deviceId,banks:(Array.isArray(input.banks)?input.banks:base.banks).filter(x=>x&&typeof x==="object").map(x=>({id:x.id||uuid(),code:text(x.code).toUpperCase(),name:text(x.name)})),customers,cardProducts,customerCards,cashbackPrograms,mccCategories,orderTypes,sourceNames,transactions,settings};
}
