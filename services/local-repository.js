...
export function canonicalize(input={}){
  ...
  const customerCards=(Array.isArray(input.customerCards)?input.customerCards:[]).filter(x=>customerIds.has(x.customerId)&&productIds.has(x.cardProductId)).map(x=>{const master=productsById.get(x.cardProductId),legacyShared=x.sharedLimitCardIds??x.sharedLimitCardId??x.sharedCardIds??x.sharedLimitCards;return {id:x.id||uuid(),customerId:x.customerId,cardProductId:x.cardProductId,bankCardProductId:x.cardProductId,cashbackCycleModeOverride:x.cashbackCycleModeOverride==="monthly"||x.cashbackCycleModeOverride==="statement"?x.cashbackCycleModeOverride:"",cardBrand:normalizeCardBrand(x.cardBrand||x.cardScheme||master?.cardBrand||master?.network),cardForm:text(x.cardForm||x.formFactor||master?.cardForm)||"Vật lý",creditLimit:parseMoney(x.creditLimit),statementDay:day(x.statementDay??x.billingDay),paymentDueDay:day(x.paymentDueDay??x.dueDay),sharedLimitCardIds:uniqueTextArray(legacyShared).map(resolveProductId).filter(id=>id&&id!==x.cardProductId),openingDate:toStorageDate(x.openingDate),expiryDate:toStorageDate(x.expiryDate),last4Digits:text(x.last4Digits).replace(/\D/g,"").slice(-4),status:text(x.status)||"active",notes:text(x.notes),createdAt:x.createdAt||now,updatedAt:x.updatedAt||now};});
  ...
  const mccCategories=(Array.isArray(input.mccCategories)?input.mccCategories:[]).map(x=>({id:x.id||uuid(),name:text(x.name||x.categoryName),codes:uniqueTextArray(x.codes??x.mcc??x.mccCodes).map(String),description:text(x.description),notes:text(x.notes)})).filter(x=>x.name);
  const mccIds=new Set(mccCategories.map(x=>x.id));
  const cashbackPrograms=(Array.isArray(input.cashbackPrograms)?input.cashbackPrograms:[]).filter(x=>productIds.has(x.bankCardProductId||x.cardProductId)).map(x=>{const unlimited=x.maxCashbackUnlimited===true||x.maxCashbackMode==="unlimited";const rate=Number(x.rate??x.cashbackRate)||0,max=unlimited?null:parseMoney(x.maxCashback??x.max);return {id:x.id||uuid(),bankCardProductId:x.bankCardProductId||x.cardProductId,name:text(x.name),startDate:toStorageDate(x.startDate),endDate:toStorageDate(x.endDate),mccCategoryIds:uniqueTextArray(x.mccCategoryIds).filter(id=>mccIds.has(id)),allMcc:x.allMcc===true,transactionMethod:text(x.transactionMethod||x.channel),rate,maxCashbackUnlimited:unlimited,maxCashback:max,eligibleTarget:unlimited?null:(parseMoney(x.eligibleTarget)||Math.round(max/(rate/100||1))),totalTarget:x.totalTarget==null||x.totalTarget===""?null:parseMoney(x.totalTarget),sharedCashbackGroup:text(x.sharedCashbackGroup||x.shared),priority:Number(x.priority)||0,notes:text(x.notes),status:text(x.status)||"active",createdAt:x.createdAt||now,updatedAt:x.updatedAt||now};});
  const programIds=new Set(cashbackPrograms.map(x=>x.id));
  const linkIds=new Set(customerCards.map(x=>x.id));
  const transactions=(Array.isArray(input.transactions)?input.transactions:[]).filter(x=>customerIds.has(x.customerId)&&linkIds.has(x.customerCardId||x.cardId)).map(x=>({id:x.id||uuid(),date:toStorageDate(x.date),customerId:x.customerId,customerCardId:x.customerCardId||x.cardId,amount:parseMoney(x.amount),mccCategoryId:mccIds.has(x.mccCategoryId)?x.mccCategoryId:"",transactionMethod:text(x.transactionMethod||x.channel),cashbackProgramId:programIds.has(x.cashbackProgramId)?x.cashbackProgramId:"",status:["completed","pending","cancelled"].includes(x.status)?x.status:"completed",notes:text(x.notes||x.note),createdAt:x.createdAt||now,updatedAt:x.updatedAt||now}));
  return {...base,...input,schemaVersion:3,deviceId:input.deviceId||base.deviceId,banks:(Array.isArray(input.banks)?input.banks:base.banks).map(x=>({id:x.id||uuid(),code:text(x.code).toUpperCase(),name:text(x.name)})),customers,cardProducts,customerCards,cashbackPrograms,mccCategories,transactions,settings:{...(input.settings||{})}};
}
