...
export class DriveRepository{
  ...
  async request(url,options={}){
    ...
    const r=await fetch(url,{...options,headers:{Authorization:`Bearer ${this.auth.accessToken}`,...options.headers}});
    if(!r.ok){
      const detail=await r.text().catch(()=>"");
      console.error("[Host Google Drive API]",{status:r.status,statusText:r.statusText,body:detail,url});
      const error=new Error(`drive-${r.status}`);
      error.code=`drive-${r.status}`;
  ...
  readFile(id){return this.request(`${API}/${id}?alt=media`);}
  async createFile(payload){const p=body(FILE,payload);return this.request(`${UPLOAD}?uploadType=multipart&fields=id,name`,{method:"POST",headers:{"Content-Type":p.type},body:p.value});}
